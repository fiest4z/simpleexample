package ru.rsatu.moderntech.zaoch.service;

import jakarta.enterprise.context.ApplicationScoped;
import jakarta.inject.Inject;
import ru.rsatu.moderntech.zaoch.mapper.DolzhnostMapper;
import ru.rsatu.moderntech.zaoch.pojo.dto.DolzhnostView;
import ru.rsatu.moderntech.zaoch.repository.DolzhnostRep;

import java.util.List;
import java.util.stream.Collectors;

@ApplicationScoped
public class DolzhnostServ {
    @Inject
    DolzhnostMapper mapper;

    @Inject
    DolzhnostRep rep;


    /**
     * Получение списка должностей
     * @return список должностей
     */
    public List<DolzhnostView> load(){

        //хагрузка данных, разделение на составляющие списка, маппинг в представление и сборка в список
        return rep.load().stream()
                .map(mapper::toDolzhnostView)
                .collect(Collectors.toList());
    }

    /**
     * Сохранение должности
     * @param model
     */
    public void save(DolzhnostView model){
        rep.save(mapper.toDolzhnost(model));    //отдача мапперу и сохранение результата
    }

}
